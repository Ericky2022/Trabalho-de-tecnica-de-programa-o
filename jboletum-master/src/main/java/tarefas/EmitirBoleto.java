...
public class EmitirBoleto implements Supplier<Map<String, String>> {
    ...
    private static int extrairLinhasDoArquivo(Path file) {
        ...
        try (var linhas = Files.lines(file)) {
            linhas.forEach(linha -> log.info("Linha: {}", linha));
            ...
            gerarCodigoDeBarras(PaymentSlip.builder().build());
    ...
    private static void gerarArquivoRetorno(String nomeArquivoRemessa, List<String> codigosDeBarra) throws FileNotFoundException {

        String retorno = new Scanner(new File("retorno.ret")).useDelimiter(".ret").next();
        log.info("Codigo de Barras gerado: {}");

    }

    private static String gerarCodigoDeBarras(PaymentSlip paymentSlip) {
        String codigoBarras = paymentSlip.getId() + paymentSlip.getDueDate()
                + paymentSlip.getPayer() + paymentSlip.getPayee() + paymentSlip.getValue();
        return codigoBarras;
    }
